...
First, we removed the last layer of the network and replaced it with our own classification or regression layer in the form of a simple linear layer that had the appropriate number of output classes for our problem (2 for regression for the 2 coordinates). Second, because, as we mentioned before, every location contained 4 images, we modified our network to perform its forward operations for the four images in tandem and then concatenate the outputs before imputing them into the classification layer. We did this after doing some research on what was the best way to compute the outputs of separate, but statistically linked images.
...
As previously mentioned, the dataset had two components. The first one was collection of folders each containing 4 images from the same location with the folders being named with a unique identifier. The second one was a CSV table with each row containing a folder ID and the corresponding image’s latitude and longitude expressed in degrees. The data was divided into a training, validation and testing folder in a 80/10/10 split and loaded during the training process when necessary. Due to the datasets (and image’s) large size, we were unable to load the whole dataset directly into memory. This had the potential of greatly decreasing training times, but we had to figure out other methods to speed up training, like using only parts of the dataset and enabling smarter learning rate schedulers.

Before the training process itself, we needed to process the data. This entailed modifying the CSV file to include data such as class affiliation, class centroid information and the image’s and centroids latitude and longitude in CRS format. Aside from this, image information such as minimum and maximum value had to be stored as well for fetching during the training process, and later, inference.

For the competition, we were provided with a dataset of around 80 000 images (20 000 separate locations of four images each), but we added our own separately procured images from Google Street View. In common deep learning wisdom, having more training data is the best form of regularization, so this was our main guideline for this.

## The model training phase

The algorithm had 3 steps. First, it would go through the training dataset in specified batches (usually 8, more if memory constraints allowed). The training step had to be as fast as possible because it entailed the vast majority of the learning phase. Due to this, haversine distance wasn’t calculated here. This not only let us skip the time consuming process of calculating the haversine distance itself, but also avoided the costly transformation of the data to a format suitable for the haversine distance functions.

After we exhausted the training dataset, we would validate our progress on the validation dataset. This is done to prevent overfitting. As the model learns on the training dataset, if it is of a sufficient size, it will essentially memorize all the information in the training dataset and learn how predict it perfectly. However, because it has never seen the data in the validation dataset, it will predict it correctly only if it has a good generalization capacity. Aside from this, we can finally calculate the haversine distance here, which is the most accurate measure of our current progress and the final performance of the model. It is worth noting that the model doesn’t learn on the validation dataset, as training is disabled here. Another thin to note is that the validation phase also functions as “checkpoint” for our model. The best performing iterations of the model are saved for later for testing and inference.

Finally, after all the training epochs are over, we test our model on the testing dataset. This dataset has never been seen by our model and is used as a final performance assessment. The best model found in the validation phase is used here.

## Hyperparameters

The image size was normally set to either 112x112 or 56x56 pixels. A smaller image size allowed for larger batches and faster training, but potentially contained less information. Batch size itself essentially regulated how accurate our training was. Perfect backpropagation would update weights only after going through all images in the training dataset. This resulted in the most accurate possible gradients, but had the downside of being very slow and often ending up in local optima. On the other end of the spectrum, training on only one set of images per batch was very fast and would easily escape local optima, but was also rather inaccurate and would sometimes converge slowly. The best of both worlds could be achieved by training on multiple images per batch, ideally as many as possible (actually, there is an upper limit to the number of batches we should use, but it is often infeasibly large). Memory and GPU limitations often don’t allow for this, so we are forced to use the largest batch size we can (8, to be precise for an image size of 112x112). Lowering the image resolution can give us more wiggle room, but setting a too large batch size runs the risk of filling up the GPU memory and crashing the training process crashing midway through.

Aside from image size and batch size, another important parameter to be adjusted was learning rate. A too large learning rate would converge the model quickly to a solution, but would lack fine tuning capabilities for that extra edge in performance. A too small learning rate would make the training process very slow and make it often end up in local minima. Ideally, we would have a large learning rate in the early staged of training and periodically reduce it as the training moves on. Momentum can also help in this. Therefore, we chose to use the Adam optimizer due to its good balance between precision and speed and a built in momentum function.

## Inference

Inference is done after the whole training phase of our model is over. It is a process of testing our best performing model on never before seen images and can be described as the training phase in reverse. Instead of seeing an image’s coordinates and learning on them, we now look at the image and only predict it’s coordinates. The true image coordinates are hidden from us and compared against our answers without us overseeing any part of the process. This is how our model will finally be tested and compared against other models to assess its performance.
