...
from utils_functions import init_message, print_df_sample
...
class GeoguesserDataModule(pl.LightningDataModule):
    ...
    def __init__(
        ...
        dataset_dirs: list[str] | list[Path],
        image_size: int,
        batch_size: int,
        dataset_frac: float,
        num_workers: int,
        ...
        **kwargs,
        ...
        df = self._load_dataframe(Path(dataset_csv))
        ...
        if not train_mean_std:
            train_image_dirs = [
                Path(dataset_dir, DatasetSplitType.TRAIN.value)
    ...
    def _load_dataframe(self, cached_df: Path) -> pd.DataFrame:
        """
    ...
    def _dataframe_create_classes(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Remove Dataframe rows for images that do not exist in any of dataset directories.
    ...
    def _get_and_fit_min_max_scaler_for_train_data(
        ...
        Args:
            df: dataframe
        """
        df_train = filter_df_by_dataset_split(
            df, self.dataset_dirs, [DatasetSplitType.TRAIN]
        )
    ...
    def _scale_min_max_crs_columns(
        self, df: pd.DataFrame, scaler: MinMaxScaler
    ) -> pd.DataFrame:
        """
        ...
        Args:
            df: dataframe
        """
        df.loc[:, ["crs_x_minmax", "crs_y_minmax"]] = scaler.transform(
            df.loc[:, ["crs_x", "crs_y"]]
        )

        df.loc[
            :, ["crs_centroid_x_minmax", "crs_centroid_y_minmax"]
        ] = scaler.transform(df.loc[:, ["crs_centroid_x", "crs_centroid_y"]])
        df.loc[
            :, ["crs_x_weighted_minmax", "crs_y_weighted_minmax"]
        ] = scaler.transform(df.loc[:, ["crs_x_weighted", "crs_y_weighted"]])

        return df

    def _get_class_to_coords_maps(
        ...
        for class_idx in range(num_classes):
            ...
            class_to_crs_weighted_map.append(
                [polygon_crs_x_weighted, polygon_crs_y_weighted]
            )
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if self.dataset_frac != 1:
            dataset_train_indices = np.random.choice(
                ...
                int(self.dataset_frac * len(dataset_train_indices)),
                replace=False,
            )
            dataset_val_indices = np.random.choice(
                dataset_val_indices,
                int(self.dataset_frac * len(dataset_val_indices)),
                replace=False,
            )
            dataset_test_indices = np.random.choice(
                dataset_test_indices,
                int(self.dataset_frac * len(dataset_test_indices)),
                replace=False,
            )
        ...
        print(
            "Train size",
            self.train_size,
            dataset_train_indices[0:5],
            dataset_train_indices[-5:],
        )
        print(
            "Val size",
            self.val_size,
            dataset_val_indices[0:5],
            dataset_val_indices[-5:],
        )
...
if __name__ == "__main__":
    ...
    #     dataset_csv=Path(PATH_DATA_COMPLETE, "data__spacing_0.5__num_class_55.csv"),
