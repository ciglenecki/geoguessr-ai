...
def filter_df_by_dataset_split(
    df: pd.DataFrame, dataset_dirs: list[Path], split_types: list[DatasetSplitType]
):
...
def get_dataset_dirs_uuid_paths(
    dataset_dirs: Union[list[Path], Path],
    dataset_split_types: Union[list[DatasetSplitType], DatasetSplitType],
):
    ...
    uuid_dir_paths = flatten(
        [
            ...
            for dataset_dir, dataset_split_type in product(
                dataset_dirs, dataset_split_types
            )
        ]
    )
